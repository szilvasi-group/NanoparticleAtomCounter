...
def get_interface(
    ...
    NP_Support_bonds = {e.capitalize(): analyzer.get_bonds(e.capitalize(),
        np_element, unique=True)[0] for e in support_elements}
    support_interfacial_indices, np_interfacial_indices = [], []
    for (support_e, bonds) in NP_Support_bonds.items():
        if bonds:
            np_interfacial_indice, support_interface_indice = zip(*[(j, i) for (i, j) in bonds])
...
def get_np_surface_by_CN(
    ...
    if not nl:
        ...
        scaling_factor=1.0
        cutoffs  = natural_cutoffs(atoms, mult = scaling_factor, **cutoff_kwargs) #set non NP atoms to zero cutoffs
        nl = NeighborList(cutoffs = cutoffs, self_interaction = False, bothways = True, skin = NL_SKIN)
        nl.update(atoms)
...
def discriminate(
    ...
    try:
        ...
        np_surface = [i for i in np_surface_plus_interface if i not in np_interface_plus_perimeter]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
            "--interface_scaling", "-is",
            type = float, default = INTERFACE_SCALING,
            help = f"""scaling factor for interfacial atoms' covalent radii.
            Default = {INTERFACE_SCALING}"""
            )
    parser.add_argument(
            "--surface_scaling", "-ss",
            type = float, default = CN_SCALING,
