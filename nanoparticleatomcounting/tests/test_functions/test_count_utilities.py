...
def test_calculate_constants():
    ...
    assert pd_molarvol == pytest.approx(8.86216261990501e+24, abs=TOLERANCE)
...
def test_alpha():
    ...
    assert alpha(90) == pytest.approx(1., abs=TOLERANCE)

def test_beta():
    for angle in [0, 180]:
        with pytest.raises(ValueError):
            beta(angle)
    assert beta(90) == pytest.approx(2., abs=TOLERANCE)
...
def test_calculate_atomic_density():
    assert calculate_atomic_density("Ag", (1,1,1)) == pytest.approx(0.1380551931635095, abs=TOLERANCE)
